...
async function run() {
  try {
    ...
    if (!ok) {
      core.setFailed('Erro ao enviar notificação par ao Google Chat')
    } else {
      core.debug(`Notificação enviada: ${name}, ${status}`)
    }
  } catch (e) {
    core.setFailed(`Erro ao enviar notificação par ao Google Chat: ${e}`)
  }
}
...
function createCard({ name, status, owner, repo, eventName, ref, actor, workflow, sha, number, collapse }) {
  ...
  if (eventType === events.pull_request) {
    ...
    eventNameFmt = 'Push'
  } else if (eventType === events.schedule) {
    eventNameFmt = 'Schedule'
  } else {
